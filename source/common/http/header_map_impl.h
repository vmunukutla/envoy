  ...
  struct StaticLookupTable : public TrieLookupTable<std::function<StaticLookupResponse(Impl&)>> {
    ...
    void finalizeTable() {
      ...
      for (const auto& header : headers) {
        this->add(header.first.get().c_str(), [&header](ImplType& h) -> StaticLookupResponse {
...
public:
  ...
  const HeaderEntry* getInline(Handle handle) const override {
    //fixfixASSERT(handle.it_->second < inlineHeadersSize());
...
private:
  ...
  static size_t inlineHeadersSize() {
    return StaticLookupTable<RequestHeaderMapImpl, RequestHeaderMap>::size() *
           sizeof(HeaderEntryImpl*);
  }
...
public:
  // fixfix Actually wire up variable inline headers.
...
private:
  ...
  static size_t inlineHeadersSize() {
    return StaticLookupTable<ResponseHeaderMapImpl, ResponseHeaderMap>::size() *
           sizeof(HeaderEntryImpl*);
  }
...
private:
  ...
  static size_t inlineHeadersSize() {
    return StaticLookupTable<ResponseTrailerMapImpl, ResponseTrailerMap>::size() *
           sizeof(HeaderEntryImpl*);
  }
